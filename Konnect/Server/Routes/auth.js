...
const bcrypt = require('bcryptjs') // bcrypt is js package whic his used to hide the password(encrypt it) and installed using npm install bcryptjs
...
router.post('/signup',(req,res)=>{ //posting data(name email pass) through request after user requests the callback function will fire
    ...
    .then((savedUser)=>{
        ...
        bcrypt.hash(password,12) // it is used to encrypt the password giving a 12 character encrypted password default value is 10
        .then(hashedpassword=>{
            const user = new User({
                email,
...
router.post('/signin',(req,res)=>{
    ...
    .then(savedUser=>{
        ...
        bcrypt.compare(password,savedUser.password) // we are compaing the passwords already in the database with the passwords entered through frontend
